...
<section id="canhan">
	<div class="container">
		<div class="row">
			...
			<div class="col-md-9 col-lg-9 cn1">
			...
			<div class="row">
				...
				<table class="table">
						<thead>		
							...
							<td>
								@if($chitietdonhang->note == "thanh toán đủ")
	               	 			<p>Đã thanh toán</p>
	               	 			<a href="{{ route('canhan.donhang.khieunai',['id' =>$chitietdonhang->id]) }}" class="btn btn-success" ">Khiếu nại</a>
	               				@elseif($chitietdonhang->note == "không đủ số tiền thanh toán")
	               				<p>Đến tận nơi thanh toán</p>
	               				@else
	               				<a href="{{ route('canhan.donhang.xacnhanthanhtoan',['id' =>$chitietdonhang->id]) }}" class="btn btn-success" onclick="alert('Đã thanh toán')" name="Click Me">Xác nhận</a>
	               				@endif
							</td>
							</tr>
							@endforeach
					...
					<div class="col-md-12">
							<div class="row">
								<div class="col-md-10">
									...
									<br>
									<p style="color: red">Chú ý : nếu đơn hàng trên 10kg quý khách có thể được giao tận nhà</p>
										<p  style="color: red">đơn hàng có thể công dồn đến khi nào đủ 10kg</p>	
								</div>
								<div class="col-md-2">
									@if($chitietdonhang->note == "thanh toán đủ" || $chitietdonhang->note == "thanh toán sau" )
                    <div class="dropdown">
                        <button class="dropbtn">Giao hàng</button>
                        <div class="dropdown-content">
                            @if($soluongsp >=2)
                            <a href="{{ route('canhan.donhang.giaohangtannha',['id' =>$chitietdonhang->id]) }}">giao hàng tận nhà</a>
                            <a href="{{ route('admin.donhang.nhantaicuahang',['id' =>$chitietdonhang->id]) }}">nhận tại cửa hàng</a>
                            <a href="{{ route('canhan.donhang.chogiaohang',['id' =>$chitietdonhang->id]) }}">chờ giao hàng(10kg)</a>
                            @else
                            <a href="{{ route('admin.donhang.nhantaicuahang',['id' =>$chitietdonhang->id]) }}">nhận tại cửa hàng</a>
                            <a href="{{ route('canhan.donhang.chogiaohang',['id' =>$chitietdonhang->id]) }}">chờ giao hàng(10kg)</a>
                            @endif
                    ...
                    @endif
								...
								<hr>
										<div class="donhang">
												<p>
													Tiền hàng(Tạm tính): {{$tienhang}} VNĐ  
													@if($chitietdonhang->status == "Nhận tại cửa hàng") 
                    								<span style="    padding-left: 243px;">Địa chỉ cửa hàng: </span>	
               										@endif
	                                            ...
	                                            <p>Cân quy đổi: {{$klqd}} Kg  <span style="margin-left: 76px;">Chiều dài: <span style="background-color: #737373;color: white;">{{$length}}</span> X Chiều rông: <span style="background-color: #737373;color: white;">{{$width}}</span> X Chiều cao: <span style="background-color: #737373;color: white;">{{$height}}</span></span></p>
	                                            <p>Phi vận chuyển TQ-VN: {{$cvc}} VNĐ</p>
											<div class="tongdonhang">
												<p>Tổng tiền hàng: {{$sum}} VNĐ </p>
