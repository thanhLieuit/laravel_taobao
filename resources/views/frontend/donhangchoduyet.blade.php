	...
	<div class="container">
		...
		<div class="col-md-3 cn1">
			...
			<div class="col-md-9 col-lg-9 cn1">
				<center><h2 style="text-transform: uppercase;">đơn hàng chờ duyệt</h2></center>	
			<div class="card2">
			...
			<div class="row">
				<h3> <p><span class="has-text-grey-light">Mã đơn hàng:</span>{{ $chitietruser->madonhang }}</p></h3>
					...
					<thead>		
						...
						<td>
							@if($chitietdonhang->status == "Còn hàng")
                                <form action="{{ route('canhan.donhang.tatcadonhang.coctien',['id' =>$chitietdonhang->id]) }}" enctype="multipart/form-data" method="post" >
                                    <input type="hidden" name="_token" value="{{csrf_token()}}">
                                    <button type="submit" class="btn btn-primary" >Đặt cọc</button>
                                </form>
                                
                                <form action="{{ route('canhan.donhang.huydonhang',['id' =>$chitietdonhang->id]) }}" enctype="multipart/form-data" method="post" >
                                <input type="hidden" name="_token" value="{{csrf_token()}}">
                                <button type="submit" class="btn btn-primary" style="background-color: #b73333" >Hủy</button>
                            @elseif($chitietdonhang->status == "chờ xác nhận")
                               <form action="{{ route('canhan.donhang.huydonhang',['id' =>$chitietdonhang->id]) }}" enctype="multipart/form-data" method="post" >
                                <input type="hidden" name="_token" value="{{csrf_token()}}">
								...
								<hr>
									<div class="donhang">
										...
										<div class="tongdonhang">
											...
											<p>Đã thanh toán: {{$coc}} VNĐ- Tiền hàng còn thiếu: {{$sumconthieu}} VNĐ</p>
										</div>
                                    </div>
							</div>
...
<script>
	jQuery(document).ready(function($) {
		$(".note_update1").change(function(event) {
			$note = $(this).val();
